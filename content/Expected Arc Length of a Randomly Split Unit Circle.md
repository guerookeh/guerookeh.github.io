...
Consider mapping its circumference onto the interval $[0, 2\pi)$, therefore it suffices to consider the points with the minimum and maximum values of the $X_i$'s to compute the length of the arc that contains $(1,0)$.

Specifically, for any three points $x_1, x_2, x_3\in[0,2\pi)$, we can calculate the length of the arc containing $(1,0)$ as, 
$$ 
\text{arc length} = \text{min}(x_1,x_2,x_3)+(2\pi-\max(x_1,x_2,x_3)),
$$
which is the sum of the distance from $0$ to the smallest point and the distance from $2\pi$ to the largest point.
...
\end{align*}
$$
Let $X_{\text{min}}=\text{min}(X_1,X_2,X_3)$ and $X_{\text{max}}=\text{max}(X_1,X_2,X_3)$ represent the minimum and maximum values of $X_i$. 

Since we want to obtain the expected value for both $X_\text{min}$ and $X_{\text{max}}$ to substitute on the formula above, we must first calculate their cumulative distribution functions (CDFs). 

Since the points are chosen independently and uniformly (i.i.d), their CDFs factor as products. Additionally, it is worth noting that $\Pr(X_i\leq x)=\frac{x}{2\pi}$, and similarly, $\Pr(X_i>x)=1-\frac{x}{2\pi}$ for the derivation below,

$$
\begin{align*}
F_{X_{\text{min}}}(x)=\Pr(X_{\text{min}}\leq x)&=1-\Pr(X_\text{min}>x) \\
&= 1-\Pr(X_1>x, X_2>x, X_3>x) \\
&= 1-\Pr(X_1>x)\Pr(X_2>x)\Pr(X_3>x) \quad (\text{i.i.d}) \\
&= 1-\left(1-\frac{x}{2\pi}\right)^3 \\
F_{X_{\text{max}}}(x)=\Pr(X_{\text{max}}\leq x)&=\Pr(X_1\leq x, X_2\leq x, X_3 \leq x) \\
&= \Pr(X_1\leq x)\Pr(X_2\leq x)\Pr(X_3 \leq x) \quad (\text{i.i.d}) \\
...
$$ 
\begin{align*}
f_{X_{\text{min}}}(x)&=\frac{d}{dx}F_{X_{\text{min}}}(x)=\frac{d}{dx}\left( 1- \left( 1-\frac{x}{2\pi} \right)^3 \right)=\frac{3}{2\pi}\left( 1-\frac{x}{2\pi} \right)^2 \\
f_{X_{\text{max}}}(x)&=\frac{d}{dx}F_{X_{\text{max}}}(x)=\frac{d}{dx}\left(\left( \frac{x}{2\pi} \right)^3 \right)=\frac{3}{2\pi}\left(\frac{x}{2\pi}\right)^2,
\end{align*}
$$
...
$$
\begin{align*}
\mathbb{E}[X_{\min}]&=\int_0^{2\pi}xf_{X_{\min}}(x)dx=\int_0^{2\pi}x\left( \frac{3}{2\pi}\left( 1-\frac{x}{2\pi} \right)^2 \right)dx=\frac{\pi}{2} \\
\mathbb{E}[X_{\max}]&=\int_0^{2\pi}xf_{X_{\max}}(x)dx=\int_0^{2\pi}x\left( \frac{3}{2\pi}\left(\frac{x}{2\pi}\right)^2\right)dx=\frac{3\pi}{2}
\end{align*}
$$
...
$$
\begin{align*}
f_{X_{\min}}(x)&=\frac{\partial}{\partial x}\left( 1-\left(1-\frac{x}{2\pi}\right)^n \right)=\frac{n}{2\pi}\left( 1-\frac{x}{2\pi} \right)^{n-1} \\
f_{X_{\max}}(x)&=\frac{\partial}{\partial x}\left( \left( \frac{x}{2\pi} \right)^n \right)=\frac{n}{2\pi}\left( \frac{x}{2\pi} \right)^{n-1}
\end{align*}
$$
Their expectations are,
$$ 
\begin{align*}
\mathbb{E}[X_{\min}]&=\int_0^{2\pi}xf_{X_{\min}}(x)dx=\int_0^{2\pi}x\left( \frac{n}{2\pi}\left( 1-\frac{x}{2\pi} \right)^{n-1} \right)dx=\frac{2\pi}{n+1} \\
\mathbb{E}[X_{\max}]&=\int_0^{2\pi}xf_{X_{\max}}(x)dx=\int_0^{2\pi}x\left( \frac{n}{2\pi}\left( \frac{x}{2\pi} \right)^{n-1}\right)dx=\frac{2\pi n}{n+1}
\end{align*}
$$
Substitute these into the formula laid out at the beginning,
$$
...
This approach is somewhat redundant considering since it considers both a minimum and a maximum in the interval. An alternative, cleaner approach for the $n$-point case is [Hagen Von Eitzen's solution in StackExchange](https://math.stackexchange.com/questions/272927/expected-length-of-arc-in-a-randomly-divided-circle).

This solution considers the arc length from $(1,0)$ to the next point in the interval $[0, 2\pi)$, which is the minimum of the $X_i$, summing both clockwise and counterclockwise components. 
